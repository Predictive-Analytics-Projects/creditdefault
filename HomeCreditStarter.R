...
#bbalance <- read.csv(paste('bureau_balance.csv',sep=","),header=TRUE);
...
docs <- str_subset(names(app_train), "FLAG_DOC")
live <- str_subset(names(app_train), "(?!NFLAG_)(?!FLAG_DOC)(?!_FLAG_)FLAG_")
inc_by_org <- tr_te %>% 
  group_by(ORGANIZATION_TYPE) %>% 
  summarise(m = median(AMT_INCOME_TOTAL)) %$% 
  setNames(as.list(m), ORGANIZATION_TYPE);

rm(app_train, app_test, fn, sum_bureau, sum_cc_balance, 
   sum_payments, sum_pc_balance, sum_prev); 
gc()
...
x <- as.data.frame(tr_te)
which(sapply(x,anyNA))
anyNA(x[,-(20:20)]) 
#---------------------------
cat("Preparing data...\n")
...
#---------------------------
read_csv("../HomeCredit/sample_submission.csv") %>%  
